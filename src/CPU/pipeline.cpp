    ...
    */
    ...
    if (branch_target_buffer && predictable) {
        if (branch_status == BRANCH_TAKEN) {
            ...
            if (!this->code->branchPredicted(this->if_id.IR)) { // throw in an extra stall...
...
*/
    ...
    if (this->div.active) {
        ...
        if (this->div.cycles == this->DIV_LATENCY) {
            ...
            if (waw(ins.type, ins.function, ins.rd)) {
                *rawreg = ins.rd;
                return WAW;
            } //new
            ...
            this->div.cycles--;
        } else if (this->div.cycles > 0) {
            this->div.cycles--;
            status = OK;
        }
        if (this->div.cycles == 0) {
            if (!this->ex_mem.active) { /*  finish divide  */
