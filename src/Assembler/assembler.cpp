...
int Assembler::second_pass(const char *line, int /* lineptr */) {
    ...
    if (instruct < 0) {
        ...
        if (!directive(2, ptr, line)) {
            if (ptr != nullptr && CODEORDATA == DATA)
                return 1;
        }
        return 0;
    }
    ...
    switch (sub) {
        ...
        case LOAD:
            ...
            if (rt < 0)
                break;
            ptr = skip(ptr, ',');
            if (eatwhite(ptr) == nullptr) break;  /* skip over ,  */
            ptr = eatwhite(ptr);
            if (*ptr == '(') w = 0;
        ...
        case 3:  // .word
            ...
            if (pass == 1) {
                if (CODEORDATA == DATA) {
                    do {
                        dataptr += STEP;
                    } while ((ptr = skipover(ptr, ',')) != nullptr);
